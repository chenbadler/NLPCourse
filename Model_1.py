...
class MEMM:
    ...
    def initialize_train(self):
        ...
        for line in self.train_data:
            ...
            for word in splitted:
                ...
                if word_and_Tag[0].lower() not in self.tags_seen_on_train_for_word:
                    self.tags_seen_on_train_for_word[word_and_Tag[0].lower()] = []
                    self.tags_seen_on_train_for_word[word_and_Tag[0].lower()].append(word_and_Tag[1])
                else:
                    if word_and_Tag[1] not in self.tags_seen_on_train_for_word[word_and_Tag[0].lower()]:
                        self.tags_seen_on_train_for_word[word_and_Tag[0].lower()].append(word_and_Tag[1])
                "saving all tags observed in train"
                if word_and_Tag[1] not in self.tags_we_saw_in_train:
                    self.tags_we_saw_in_train.append(word_and_Tag[1])
    ...
    def build_features_matrix_train(self):
        ...
        for i in range(len(self.all_sentences_train)):  #for each sentence in train
            ...
            for word_index in range(len(sentence)):     #for each word in sentence
                ...
                word_matrix = csr_matrix((data, (rows_index, cols_index)),
    ...
    def make_confusion_matrix(self, x_true, x_pred):
        ...
        pd.DataFrame(mat, columns=tags_print, index=tags_print).to_csv('conf_matrix_model_1', sep='\t')
        n = 10
    ...
    def write_predictions_to_file(self):
        ...
        file_name = 'comp_m1_303091698.wtag'
    ...
    def inference_procedure(self):
        ...
        self.make_confusion_matrix(self.all_tags_test, self.test_predictions)
